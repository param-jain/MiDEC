...
class SignupScreen extends Component {
    ...
    navigateToSignUpDetails() {
        ...
        // const url = 'http://midec-dev.ap-south-1.elasticbeanstalk.com:8181/midec/usrg/create?userEmail='+`${this.props.email}`+'&password='+`${hashedPassword}`+'&userType=advisee'
           const url = 'http://midec-dev.ap-south-1.elasticbeanstalk.com:8181/midec/usrg/chngpwd/'+`${JSON.parse(this.state.currentLoggedInUser).referenceTx}`+ '?password=' +`${hashedPassword}`;
        ...
        const postData = {
          ...
          fetch(url, {
            ...
            .then(res => {
                    ...
                    this.setState({isAuthenticating: false, errorMessage: res._bodyInit});
                    }
            })
            .catch(err => {
    ...
    enableNextButton = (oldPassword, password, verifyPassword) => {
        if (
            (password.length<8)||
            (oldPassword === global.currentLoggedUserPassword)||
    ...
    renderHeader = () => {
        return(
            <Header
              ...
              centerComponent={{ text: 'Account Settings' , style: { color: '#fff',fontSize: 18, fontWeight: 'bold' }  }}
